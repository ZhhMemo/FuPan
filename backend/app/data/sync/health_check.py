...
**涨跌幅校验口径（Q5，已获裁决通过；避免误报，务必按此理解）**：
A 股真实数据中，以下情形单日涨跌幅会**合法地**超过板块限制，若不剔除会产生大量误报。
因此 ``check_pct_change`` 只在同时满足下述**四项排除条件**、并且叠加 **0.5% 容差** 的行上判定超限：

1. **后复权口径**：以 ``close × adj_factor``（后复权）计算涨跌幅，消除除权/配股造成的名义跳空；
2. **排除除权除息 / 配股日**：要求相邻两行 ``adj_factor`` **相等**（``prev_af = adj_factor``）；
   复权因子变动日即除权日，其上不复权价的跳空是**制度性**的，非异常；
3. **排除停牌复牌**：要求相邻交易间隔 ``date - prev_date ≤ max_gap_days``（默认 10 天）；
   长期停牌复牌当日可合法大幅波动；
4. **排除制度生效前**：仅统计 ``date ≥ pct_check_start``（默认 ``1996-12-16``）的行；
   此前 A 股尚未统一实行 ±10% 涨跌停板制度。

在上述口径之上再叠加 **``pct_tolerance``（默认 0.5%）容差**，抵消「涨跌停价四舍五入到分」
引入的微小偏差。经过这四项排除 + 容差后，**任何剩余超限即为真实异常**，应告警；
有异常则明确输出，不静默（对齐 §21「同步失败必须可见」）。

> 说明：本口径同时用于「判定是否为异常」，与涨跌停**预计算**的口径严格区分——
> 预计算按板块**制度分段**（主板 ±10% / 创业板 2020-08-24 起 ±20% / 科创板 ±20% / 北交所 ±30%），
> 见 ``precompute_limit`` 与 ``models.limit_pct_of``。
"""
